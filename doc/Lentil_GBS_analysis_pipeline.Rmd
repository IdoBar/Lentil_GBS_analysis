...
subtitle: "Genome-by-Sequencing (ddRAD) Data Analysis Pipeline"
author: "Ido Bar"
...
pacman::p_load(char = c("tidyverse", "captioner", "knitr", "pander", "DT", "foreach", "doFuture"))
# Font Format
...
### Analysis Approach
1. Assess `r fontFmt("Stacks2")` _de novo_ parameters for maximum coverage and minimum error rates  
2. Combine each parent read files to one file (to increase coverage for them)
3. Call variants using `r fontFmt("Stacks2")` _de novo_ pipeline using optimal parameters  
4. Map catalog back to the lentil genome (v1.2; UoS)
...
7. Perform linkage mapping and order markers inside each chromosome/linkage group  
...
### Stacks pipelines
AGRF provided the results of the `r fontFmt("Stacks")` *de novo* pipeline, run as a general population of samples. Alternatively, run `r fontFmt("Stacks2")` `ref_map.pl` or `denovo_map.pl` pipelines.  
Other alternatives to `r fontFmt("Stacks")` are `r fontFmt("Heap")` or `r fontFmt("TASSEL")` [@glaubitz_tassel-gbs:_2014] 

### Stacks2 pipeline
A new version of `r fontFmt("Stacks")` (v2.0b) is now available and the recommended approach, even when a reference genome is available, is to incorporate the reference genome alignment information by building loci _de novo_ and subsequently integrating the alignments of the consensus sequences into their _de novo_ dataset [@ParisLostparameterspace2017]. 

#### Assess errors in read files
`kmer_filter` in `r fontFmt("Stacks")` was used to visualise the error profiles of the cleaned RAD-seq reads and the resulting K-mer frequency distributions were plotted. It seems that the overall error rate is low (most rare k-mers appear only once), but the coverage is also fairly low. 
...
A subset of samples was selected, including the parent replicates (non-combined) and 20 other randomally selected genotypes.  A _de novo_ pipeline was run several times with the selected samples, varying just one parameter with each parse of the program. For the primary analysis, we varied the `ustacks` **m** parameter from 3 to 7, the `ustacks` **M** parameter from 1 to 8 and the `cstacks` **n** parameter as $n=M\pm\ 1$, while keeping all other parameters consistent (`r fontFmt("Stacks2")` defaults are m3, M2 and n0), as described by [@ParisLostparameterspace2017; @RochetteDerivinggenotypesRADseq2017; @Mastretta-YanesRestrictionsiteassociatedDNA2015a].
...
Replicate samples were used to estimate the rates of three types of errors:  
...
For comparison, `r fontFmt("Stacks2")` default parameters (m3, M2 and n0) resulted in a combined score of `r default_rank$Score` and was ranked #`r default_rank$Rank` out of `r nrow(param_table)` possible combinations. 


#### Integrated de novo and genome-mapped approach
Once identified, the optimal _de novo_ parameters were used for another _de novo_ run with all the samples (this time combining the parent read files to increase coverage). Once the _de novo_ pipeline was completed, the final catalog loci (now stored in `catalog.fa.gz`) were aligned to the genome with `r fontFmt("GSNAP")` version 2018-03-25 [@WuGMAPGSNAPGenomic2016a]. The `stacks_integrate_alignments` script was then used to integrate the alignments back into the `gstacks` files, followed by another `populations` run to assign genomic coordinates for each locus in the output files. (see [discussion](https://groups.google.com/d/msg/stacks-users/ZsAnJXCs8uo/P2BjXCCsAwAJ)). 
...
ls -1 *.bam | gawk '{sample=gensub(".bam", "", "1", $1); printf "%s\t%s\n", sample, $1}'  > bams_list.txt
 
echo 'cd $PBS_O_WORKDIR; pigz -cd ~/scratch/data/reference_genomes/Lens_culinaris_v1.2/Lensculinaris_1.2b_genes.gff3.gz > Lensculinaris_1.2b_genes.gff3; qualimap multi-bamqc -gff Lensculinaris_1.2b_genes.gff3 -r -c -d bams_list.txt -outdir qualimap_report; rm Lensculinaris_1.2b_genes.gff3' | qsub -V -q qweek_s -l select=1:ncpus=12:mem=8GB,walltime=20:00:00 -m be -M i.bar@griffith.edu.au -N BT2_qualimap # 4759817.pbsserver
```
...
SnpSift filter 'AF>0.1' ref_stacks2_populations.snps.vcf | SnpSift gtfilter -gv './.' 'GQ<20 || DP<7 || DP>500'   > filtered_VCFs/ref_stacks2_populations.snpsift.vcf
...
#### OneMap
Alternatively, the filtered VCF files (one for each `r fontFmt("Stacks2")` combination of parameters) were processed by an in-house `r fontFmt("R")` script `Lentil_GBS_Stacks2_linkage_map_unix.R`, to estimate the optimal LOD score and maximum recombinant factor (`max.rf`) to construct a linkage map with markers well distributed across the genome. For all the denovo combinations, the optimal parameters were found to be `LOD=5` and `max.rf=0.35`. The linkage map was constructed using the "Kosambi" method [@KosambiD.D.estimationmapdistances2012], implemented in `r fontFmt("OneMap", custom_font)` v`r packageVersion("onemap")` [@margarido_onemap:_2007] package, while removing any SNPs deviating from a 1:1 segregation pattern. The linkage map was then manually evaluated to assess its concordance with the genomic coordinates (after consolidating orphan contigs into their parent chromosomes). 
>96% of the markers in the linkage groups matched their genomic coordinates at the chromosome level.

```{bash process_vcfs}
# prepare commands
NCPUS=8
MARKER_GROUPS="chr" # choose chr for chromosome-based or 'lg' for denovo linkage-based
JOB_NAME="process_vcfs_$MARKER_GROUPS"

cd ~/data/Lentil_GBS/AGRF_CAGRF14978_CB2YGANXX_gbs/GBS_analysis
find ~/data/Lentil_GBS/AGRF_CAGRF14978_CB2YGANXX_gbs/Stack2_vcf_files -name "*.vcf" | sort -r | gawk -v ncpus=$NCPUS -v mk_group=$MARKER_GROUPS '{printf "cd $PBS_O_WORKDIR; Rscript Lentil_GBS_Stacks2_linkage_map_unix.R \"%s\" -n %s --lod 5 --max_rf 0.35 --group %s\n", $0, ncpus, mk_group}' > ${JOB_NAME}.cmds
# Prepare PBS script
echo '#!/bin/bash 
#PBS -V
#PBS -l select=1:ncpus=8:mem=16GB,walltime=90:00:00
#PBS -m be
#PBS -M i.bar@griffith.edu.au
#PBS -q qweek_s
 
cd $PBS_O_WORKDIR
gawk -v ARRAY_IND=$PBS_ARRAY_INDEX "NR==ARRAY_IND" $CMDS_FILE | bash' > ${JOB_NAME}.pbs

# submit the jobs to the scheduler
JOBS_NUM=`wc -l ${JOB_NAME}.cmds | gawk '{print $1}'`
qsub -J1-${JOBS_NUM} -N ${JOB_NAME:0:11} -v CMDS_FILE=${JOB_NAME}.cmds ${JOB_NAME}.pbs 

```

```{r vcf_comparison, eval=TRUE, message=FALSE}
pacman::p_load_gh(c("augusto-garcia/onemap")) # "thierrygosselin/radiator", "augusto-garcia/onemap", "IdoBar/onemap@patch-3"
# Onemap requires R>=3.4 and Rhtslib and zlibbioc - install through conda on the server
...
pacman::p_load(char=package_list)
# Import to OneMap 
filtered_vcf_files <- list.files("./data/intermediate_files", ".+miss10.clean.vcf", full.names = TRUE)
# read both files and print output
# registerDoFuture()
# plan(multisession, workers = 2)
registerDoSEQ() # to run sequentially
y <- foreach(f=filtered_vcf_files) %dopar% {
  invisible(capture.output(onemap_file <- onemap::onemap_read_vcfR(vcfR::read.vcfR(f), cross = "ri self",
                                parent1 = "ILL6002_RF", 
                                parent2 = "ILL7537_RF")))
  
  cat(sprintf("Structure of filtered VCF file %s:\n", f)) 
  print(onemap_file)
}
...
#### SNP annotation 
Physical (chromosomal) locations of the QTL peaks were determined and SNPs and genes were extracted from a range of 2 Mbp up and downstream of these locations. Functional annotation determined the effect of each SNP on the underlying genes.  
...
<!-- #### Gene Model Prediction from Lentil genome  -->
