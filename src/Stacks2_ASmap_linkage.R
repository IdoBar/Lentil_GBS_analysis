...
vcf_file <- "../Stacks_vcf_files/stacks_M3m4n3_populations.snps.vcf"  # ref_stacks2_populations.snps.vcf
group_markers <- "LG"
vcf_basename <- tools::file_path_sans_ext(basename(vcf_file))
  
source("./Stacks2_OneMap_linkage_map/src/vcf_filtration.R")
...
mapBC1 <- subset(asmap, ind = sg$miss < sum(qtl::nmar(asmap))*0.2)
...
profileMark(mapRIL5, stat.type = c("seg.dist", "prop", "dxo", "recomb"),
            layout = c(1, 5), type = "l")
heatMap(mapRIL5, lmax = 30, chr = "L.6")
nmar(mapRIL5)
chrlen(mapRIL5)
# Split linkage group and merge with another (based on heatmap)
mapRIL5_split <- breakCross(mapRIL5, split = list(`L.6` = "660328:86:-"), suffix = "alpha", sep = "")
mapRIL5_merged <- mergeCross(mapRIL5_split, merge = list(`L.11` = c("L.11", "L.6B"), 
                             `L.1` = c("L.9", "L.1"), `L.7`=c("L.6A", "L.7")))
...
pg1 <- profileGen(mapRIL5b, bychr = FALSE, stat.type = c("xo", "dxo","miss"), 
                  id = "Genotype", xo.lambda = 14, layout = c(1, 3), lty = 2, cex = 0.7)3
...
marker_map_sum <- marker_map_df %>% group_by(LG_name) %>% summarise(num=n())
